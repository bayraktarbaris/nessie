...
public abstract class PerContentDeleteExpired {
  ...
  public DeleteSummary expire() {
    ...
    LOGGER.info("Start identifyLiveFiles with filter {}", filter.toString());
    long start = System.currentTimeMillis();
    long identifiedLiveFiles = identifyLiveFiles(filter, addBaseLocation);
    LOGGER.info("Elapsed time for identifyLiveFiles is {}ms" , System.currentTimeMillis() - start);
      ...
      .map(
        baseLocation -> {
          try (Stream<FileReference> fileObjects = identifyExpiredFiles(filter, baseLocation)) {
            ...
            DeleteSummary deleteSummary = expireParameters().fileDeleter().deleteMultiple(baseLocation, fileObjects);
            LOGGER.info("Delete finished for baseLocation:{} within {}ms ", baseLocation, System.currentTimeMillis() - startDelete);
