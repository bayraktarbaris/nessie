  ...
  public Stream<FileReference> listRecursively(StorageUri path) throws NessieFileIOException {
    ...
    if (supportsBulkAndPrefixOperations(path)) {
      ...
      try {
        ...
        LOGGER.debug("Listing prefixes completed: {} within: {}ms", basePath, System.currentTimeMillis() - startListPrefix);
      } catch (Exception e) {
      ...
      return StreamSupport.stream(fileInfos.spliterator(), false)
        .map(
          f -> {
            ...
            LOGGER.debug("StorageUri.of completed: {} within: {} ms", f.location(), System.currentTimeMillis() - startStorageUri);
    ...
    LOGGER.debug("Listing Hadoop completed: {} within: {}ms", basePath, System.currentTimeMillis() - startStorageHadoop);
